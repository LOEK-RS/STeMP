	...
	shiny::moduleServer(id, function(input, output, session) {
		...
		session$onSessionEnded(function() {
			old_files <- list.files(
				output_dir,
				pattern = paste0("^protocol_preview_", session$token, ".*\\.pdf$"),
				full.names = TRUE
			)
		...
		shiny::observeEvent(input$preview_pdf, {
			...
			old_pdfs <- list.files(
				output_dir,
				pattern = paste0("^protocol_preview_", session$token, "_.*\\.pdf$"),
				full.names = TRUE
			)
			...
			temp_pdf_file <- file.path(
				output_dir,
				paste0("protocol_preview_", session$token, "_", as.integer(Sys.time()), ".pdf")
			)
